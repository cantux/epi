package DetectCycleOptimizedUnionFind;

    // A union by rank and path compression
// based program to detect cycle in a graph
class DetectCycleOPTUF
{
    int V, E;
    Edge[] edge;

    DetectCycleOPTUF(int nV, int nE)
    {
        V = nV;
        E = nE;
        edge = new Edge[E];
...
// a given graph contains cycle or not
    int isCycle(DetectCycleOPTUF graph)
    {
        int V = graph.V;
        int E = graph.E;
...
1-----2 */
        ...
        int V = 3, E = 3;
        DetectCycleOPTUF graph = new DetectCycleOPTUF(V, E);
